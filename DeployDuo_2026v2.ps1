...
try {
    ...
    $gpoBackup = Get-GPOBackup -Path $gpoExtractDir -ErrorAction Stop |
        Sort-Object -Property BackupTime -Descending |
        Select-Object -First 1

    if (-not $gpoBackup) {
        throw "No GPO backups were found in $gpoExtractDir"
    }

    Import-GPO -Path $gpoExtractDir -BackupId $gpoBackup.Id -TargetName $gpoName -CreateIfNeeded -ErrorAction Stop | Out-Null
    Write-Log ("Imported GPO backup '{0}' ({1}) into '{2}'" -f $gpoBackup.DisplayName, $gpoBackup.Id, $gpoName)
} catch {
